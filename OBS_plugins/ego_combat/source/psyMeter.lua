...
local function updateCrop(player_id, use_flash)
    ...
    local source = obs.obs_get_source_by_name(player.psy_source)
    if source == nil then return end

    local frame_index = use_flash and flash_frame_index or player.psy
    ...
    local filter = obs.obs_source_get_filter_by_name(source, "PsyCrop")
    if filter ~= nil then
        obs.obs_source_update(filter, settings)
        obs.obs_source_release(filter)
    else
        print("[ERROR]<psyMeter> Could not find filter 'PsyCrop' on source '"..player.psy_source.."'")
    end
...
-- Public API
function PsyMeter.setPsyMeter(player_id, value)
    local player = PlayerReg.get(player_id)
    if not player then return end
...
function PsyMeter.setSourceName(player_id, source_name)
    PlayerReg.setPsySource(player_id, source_name)
end
